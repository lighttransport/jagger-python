...
class PyToken {
 public:
  ...
  std::string feature() const { return std::string(_feature); }

 private:
  std::string _surface;

  // TODO: Use string_view when compiled with C++17
  const char *_feature{nullptr};
};

class PyJagger {
 public:
  PyJagger() = delete;
  PyJagger(const std::string &model_path) : _model_path(model_path) {
    _tagger.read_model(model_path);
  }

  ///
  /// Tokenize single-line string.
  ///
  std::vector<PyToken> tokenize(const std::string &src);
  ...
  /// cores) to use.
  ///
  std::vector<std::vector<PyToken>> tokenize_batch(const std::string &src,
                                                   uint32_t threads = 0);

 private:
  uint32_t _threads{0};  // 0 = use all cores
  std::string _model_path;
  jagger::tagger _tagger;
};
